<script>
	...
	// A clean regex parser to turn markdown bold and links into real HTML anchors
	function parseMarkdown(text) {
		if (!text) return '';
		let html = text
			// Escape plain HTML strings to stay safe
			.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;")
			// Convert **bold** text
			.replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>')
			// Convert [Link Text](URL) into a clickable anchor element
			.replace(/\[(.*?)\]\((.*?)\)/g, '<a href="$2" target="_blank" class="text-white underline font-semibold hover:text-blue-200 decoration-white"> $1 </a>');
		
		// If the assistant message contains links, make sure the text link styling fits the assistant background card color
		return html;
	}
...
<style>
	...
	/* Styles to ensure the custom parsed markdown HTML links look beautiful inside the message bubbles */
